export class Monster {
  constructor(path, monsterImages, level, type) {
    ...
    if (this.type == 'treasureGoblin') {
      this.speed = 2; // 몬스터의 이동 속도
      this.image = monsterImages; // 몬스터 이미지
    } else {
      this.speed = 1;
  ...
  init(level) {
    if (this.type == 'treasureGoblin') {
      ...
      this.maxHp = 100 + 50 * level; // 몬스터의 현재 HP
  ...
  move(base) {
    if (this.currentIndex < this.path.length - 1) {
      ...
      this.hp = -999; // 몬스터는 이제 기지를 공격했으므로 자연스럽게 소멸해야 합니다.
      return isDestroyed;
    }
  }
